...
- [x] Phase 10 — Reminder foundation
  - [x] Timezone + quiet hours ended up **not** needing a `users` table migration: `SettingsService` already has a generic per-user key-value store (`AppSetting`, the same mechanism backing `workingHours`/`sleepHours`), so `timezone` (IANA string, default UTC, validated via `ZoneId.of`) and `quietHours` (nullable `{start,end}` window, reusing the existing `TimeWindow` type) were added as two more settings keys instead -- lower risk than a new migration, and consistent with how the rest of Settings already works. `getTimezoneForUser(userId)`/`getQuietHoursForUser(userId)` variants were added alongside the existing current-user versions specifically for the reminder job, which runs outside any request/security context and can't use `CurrentUserService`.
  - [x] `model/Reminder` + `model/NotificationOutboxEntry` (`V39__create_reminders.sql`, `V40__create_notification_outbox.sql`) -- a real transactional-outbox pair, the first `@Scheduled` jobs anywhere in this codebase (`@EnableScheduling` added to the application class)
  - [x] `reminder/ReminderService.produceReminders()` (every 60s) scans every user's tasks (due today / follow-up today) and habits (reminder enabled, not yet met today) *in that user's own timezone*, creates idempotent `Reminder` rows (a re-run within the same day never duplicates), and activates any whose time has arrived into a `NotificationOutboxEntry` (unique on reminder+channel, so a job re-run can't double-enqueue either). Quiet hours defer the computed time to the window's end, correctly handling both a same-day window and an overnight wrap (e.g. 22:00-07:00). `dispatchNotifications()` (every 30s) marks pending outbox entries delivered -- honestly, for the only channel implemented (IN_APP) "delivery" just means becoming visible via the API, so it can't actually fail; the attempts/backoff fields exist for a future channel that can.
  - [x] `GET /api/v1/notifications`, `GET /api/v1/notifications/unread-count`, `PATCH /{id}/read`, `PATCH /{id}/snooze` (creates a fresh `Reminder` at the new time and marks the original `SNOOZED` + the outbox entry read -- snoozing reschedules, it doesn't pause a timer)
  - [x] Scheduled jobs are disabled in the `local-test` profile (`app.reminders.scheduling-enabled=false`) so a long-lived `@SpringBootTest` context can't fire them mid-assertion; they default to enabled everywhere else
  - [x] Frontend: Settings gained a "Timezone & quiet hours" section (timezone `<select>` populated from `Intl.supportedValuesOf('timeZone')`, quiet-hours checkbox + start/end time inputs); a bell icon with an unread-count badge sits in the header next to Quick Add, polling `/unread-count` every 60s, opening a popover listing real notifications with relative timestamps and a "Snooze 1h" action per unread item
  - [x] Test coverage: `ReminderServiceTest` (15 -- reminder creation for each kind, no-duplicate-per-day, quiet-hours deferral, activation into an outbox entry, idempotent re-activation, dismissal when the source task/habit no longer exists, scheduling-disabled short-circuit for both jobs, dispatch, snooze, mark-read), `NotificationControllerTest` (5), `SettingsServiceTest` (+7 for timezone/quiet-hours), `NotificationInbox.test.tsx` (2)
  - [x] Verified end-to-end in a real browser against local Postgres + backend with scheduling actually enabled: set timezone to America/Los_Angeles and quiet hours in Settings, created a task due today, confirmed via direct DB query that the producer computed the correct reminder time (9am PDT converted to exactly 16:00 UTC -- the timezone math is right), backdated it to simulate the time arriving, watched the next producer+dispatcher cycle turn it into a real notification with the unread badge, correct title/body/link, and a working "Snooze 1h" that created a new reminder and marked the original snoozed. Zero console/page errors throughout.
  - **Scope decisions**: only the `IN_APP` channel is implemented -- no `BROWSER_PUSH`/service-worker infra, and the inbox is polling-based (60s) rather than SSE/WebSocket, both explicitly flagged as acceptable v1 fallbacks in the original plan. `TASK_START`/`DAILY_PLANNING`/`WEEKLY_REVIEW` reminder kinds from the original brief were left out: `Task` has no time-of-day field to anchor a "start" reminder to (the Scheduler/`TaskSchedule` mechanism is a separate concept not wired in here), and Weekly Review already has its own in-app nudge from Phase 9. The existing purely-client-side `useHabitReminders`/`HabitReminderToasts` mechanism (with its silent/gentle/standard/persistent styles) was **kept running as-is** rather than migrated onto the new server-driven `HABIT` reminder kind -- both now exist side by side. Retiring the client-only path in favor of the server-driven one is real follow-up work, not done here; risking regressions in a working, styled habit-reminder UX under this session's time constraints wasn't worth it for a foundation phase. All noted in `BACKLOG.md`.
