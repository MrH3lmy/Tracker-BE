...
**Bucket provisioning**: `docker compose up` runs a one-shot `minio-init` service (the `minio/mc` image) after `minio` reports healthy, which creates the local dev bucket with an idempotent `mc mb --ignore-existing` and then exits; `app` waits for it to complete successfully before starting. This mirrors how a real deployment provisions buckets out of band (Terraform/CloudFormation/etc.) rather than granting the application CreateBucket permission. `S3AttachmentStorage` also still attempts its own best-effort bucket auto-create at startup as a fallback (e.g. for `mvn spring-boot:run` against a bare MinIO without the Compose stack) - that attempt, and every real put/get/delete call, is unaffected by object storage being temporarily unreachable: see the next paragraph.

**Startup resilience**: object storage is optional infrastructure for the core app - a MinIO/S3 outage (DNS failure, connection refused, timeout) at startup or at runtime never fails Spring context creation or takes down unrelated features (auth, tasks, planning, notes without attachments, etc.). A real attachment upload/download/delete attempted while storage is unreachable still fails loudly and explicitly (never silently "succeeds") with `AttachmentStorageException`. Correspondingly, `AttachmentStorageHealthIndicator`'s contribution to `/actuator/health` is disabled by default (`management.health.attachmentStorage.enabled=false` in `application.properties`, same precedent as the existing `management.health.redis.enabled=false`) so a storage outage doesn't flip the Docker `HEALTHCHECK`/liveness/readiness probes to DOWN and restart-loop an otherwise-healthy app; flip it on if a given deployment's readiness semantics should require storage.
